...
% angle(N,:) = randn(1,3);
% 
% R(:,:,1) = angle2dcm(angle(1,1),angle(1,2),angle(1,3));
% R(:,:,N) = angle2dcm(angle(N,1),angle(N,2),angle(N,3));
% 
% warp(1,:) = GenerateDQfromRt(R(:,:,1),t(1,:));
% warp(N,:) = GenerateDQfromRt(R(:,:,N),t(N,:));
% for i = 1:N
...
for k = 1:7
    ...
    angle3d(1,:) =  [ a  50   0] * pi / 180;
    angle3d(N,:) =  [-a -50  -0] * pi / 180;

    R3d(:,:,1) = angle2dcm(angle3d(1,1),angle3d(1,2),angle3d(1,3));
    R3d(:,:,N) = angle2dcm(angle3d(N,1),angle3d(N,2),angle3d(N,3));
